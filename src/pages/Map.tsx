...
function isGraphicHit(
  r: __esri.HitTestResult["results"][number]
): r is __esri.GraphicHit {
  return "graphic" in r;
}
...
export default function Map() {
  ...
  useEffect(() => {
    if (!mapDiv.current || viewRef.current) return; // StrictMode 2nd mount
    ...
    const webmap = new WebMap({
      portalItem: { id: import.meta.env.VITE_MAP_ID as string },
    });
    webmap.when(() => {
      webmap.allLayers.forEach((l) => {
        if (l.type === "feature") (l as __esri.FeatureLayer).outFields = ["*"];
      });
    });
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1 min-h-0">
        <div ref={mapDiv} className="flex-1 min-h-0" />
        <RightSidebar />
      </div>
      <EditorPanel />
    </div>
