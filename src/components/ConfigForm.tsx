...
type ConfigState = {
  ...
  connectionType: "Access Connection" | "SQL Connection";
  SQLInstance: string;
  databaseName: string;
  SQLUser: string;
  SQLPass: string;
...
export default function ConfigForm({
  ...
  onClose?: () => void; // will be called after successful save
}) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setCfg((p) => ({ ...p!, [e.target.name]: e.target.value }));

  const handleRadio = (e: React.ChangeEvent<HTMLInputElement>) =>
    setCfg((p) => ({ ...p!, connectionType: e.target.value as any }));
  ...
  const isPreset = cfg && TEMPLATE_OPTS.includes(cfg.template as any);

  if (!cfg) return <p className="p-8">Loading configuration…</p>;

  return (
    <div className="max-w-xl mx-auto my-4 space-y-6 px-4 overflow-y-auto max-h-[70vh]">
       
      <h1 className="text-2xl font-semibold">Edit Configuration</h1>

      {/* API key + exec path */}
      {[{ n: "apiKey", label: "API Key" }, { n: "posmExecutablePath", label: "POSM Executable Path" }].map(
        ({ n, label }) => (
      ...
      <div>
        ...
        <div className="space-x-4 mt-1">
          {(["Access Connection", "SQL Connection"] as const).map((opt) => (
            <label key={opt}>
              <input
                type="radio"
                value={opt}
                checked={cfg.connectionType === opt}
                onChange={handleRadio}
              />{" "}
              {opt === "Access Connection"
                ? "POSM truck – Access"
      ...
      {/* SQL block */}
      {cfg.connectionType === "SQL Connection" && (
      ...
      {/* POSM Template */}
      <div>
        <span className="font-medium">POSM Template</span>
        <select
          className="mt-1 w-full border rounded px-2 py-1"
          value={isPreset ? cfg.template : "Custom…"}
          onChange={(e) => {
        ...
        {cfg.template === "" && (
          <div className="mt-2 flex gap-2">
            ...
            <button
              ...
              onClick={() =>
                setCfg((p) => ({ ...p!, template: draftTemplate.trim() }))
              }
            >
              OK
            </button>
      ...
      {/* map + id */}
      {[{ n: "mapId", label: "Web Map ID" }, { n: "idField", label: "Unique ID Field" }].map(
        ({ n, label }) => (
          <label key={n} className="block">
            <span className="font-medium">{label}</span>
            <input
